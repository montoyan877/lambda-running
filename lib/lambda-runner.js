...
try {
  ...
  try {
    ...
    console.log('tsconfig-paths module found, alias paths like @/* will be supported');
  } catch (err) {
    console.warn('tsconfig-paths module not found. Path aliases like @/* may not work.');
    console.warn('Install it with: npm install --save-dev tsconfig-paths');
    ...
    if (tsconfigPathsAvailable) {
      ...
      try {
        ...
        if (tsconfig.compilerOptions && tsconfig.compilerOptions.paths) {
          console.log('Path aliases detected in tsconfig.json, enabling support');
  ...
  try {
    ...
    if (isTypeScript) {
      ...
      if (fs.existsSync(dirTsConfigPath) && path.dirname(absolutePath) !== process.cwd()) {
        console.log(`Using TypeScript configuration from handler directory: ${dirTsConfigPath}`);
