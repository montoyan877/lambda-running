...
function getEventStoreDir() {
  ...
  try {
    ...
    if (!fs.existsSync(projectEventStoreDir)) {
      fs.mkdirSync(projectEventStoreDir, {
        recursive: true,
      });
    }
    ...
    console.warn(
      `No se puede usar el directorio del proyecto para almacenar eventos. Usando el directorio home.`
    );

    // Asegurar que el directorio en el home exista
    if (!fs.existsSync(homeEventStoreDir)) {
      fs.mkdirSync(homeEventStoreDir, {
        recursive: true,
      });
    }
  ...
  if (fs.existsSync(eventPath)) {
    try {
      ...
      return {
        ...
        timestamp: fs.statSync(eventPath).mtime.toISOString(),
        data,
        // Ahora data contiene directamente el JSON del evento
        path: eventPath,
      };
    } catch (error) {
